...
use crate::types::*;
use crate::utility::{RefSafe,URP,URPConvert,FPRef};
pub use crate::utility::{FP,RVA,CChar,Error,Result,AsOsStr};
...
// PRIVATE
	...
	fn resolve_rva<T>(&self, rva: RVA<T>) -> Result<FP<T>> {
		...
		Ok(r#try!(self.resolve_rva_raw(rva+0u32,length,None)).offset(0))
	}
...
// PUBLIC
	pub fn new(data: &'data [u8]) -> Result<Pe<'data>> {
		...
		let pe_oh=match sig {
			OH_SIGNATURE_PE32 => {
				...
				PeOptionalHeader::Pe32(r#try!(data.ref_at(pe_oh_fp.offset(0))))
			},
			OH_SIGNATURE_PE32P => {
				...
				dd_size=pe_header.size_of_optional_header-s;
				pe_dd_fp=pe_oh_fp.offset(s as u32);
				PeOptionalHeader::Pe32Plus(r#try!(data.ref_at(pe_oh_fp.offset(0))))
			},
		...
		let pe_dd=r#try!(data.ref_slice_at(pe_dd_fp,n));

		let pe_sec_fp=pe_dd_fp.offset(n*(size_of::<DataDirectory<u32>>() as u32));
		let pe_sec=r#try!(data.ref_slice_at(pe_sec_fp,pe_header.number_of_sections as u32));
	...
	pub fn get_exports(&self) -> Result<Exports> {
		let ddir=r#try!(self.get_directory::<ExportDirectory>());
		if (ddir.size as usize)<size_of::<ExportDirectory>() {
			return Err(Error::InvalidSize);
		}
		Ok(Exports{pe:self,ddir:ddir,edir:r#try!(self.ref_at(ddir.virtual_address))})
	}

	pub fn get_relocations<'pe>(&'pe self) -> Result<RelocationIter<'pe,'data>> {
		let ddir=r#try!(self.get_directory::<RelocationBlock>());
...
impl<'pe,'data: 'pe> Exports<'pe, 'data> {
	...
	pub fn lookup_symbol(&self, symbol: &str) -> Result<ExportAddress<'data>> {
		let pos=r#try!(r#try!(self.get_names()).iter().position(|&name_rva|{
			self.pe.ref_cstr_at(name_rva).ok().map_or(false,|cstr|cstr.as_os_str()==symbol)
		}).ok_or(Error::SymbolNotFound));
		let ordinal_offset=r#try!(self.get_ordinal_offsets())[pos];
		let export=r#try!(r#try!(self.get_export_addresses()).get(ordinal_offset as usize).ok_or(Error::ExportNotFound));
...
impl<'pe,'data: 'pe> RelocationIter<'pe,'data> {
	fn advance(&mut self) -> Result<(RVA<()>,&'data [Relocation])> {
		let rblock=r#try!(self.pe.ref_at(self.next_rblock));
		let relocs: &[Relocation]=r#try!(self.pe.ref_slice_at(self.next_rblock.offset(size_of::<RelocationBlock>() as u32),rblock.block_size/2));
